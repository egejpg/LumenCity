...
const EarthGlobe = dynamic(() => import("@/components/EarthGlobe"), {
  ssr: false,
  loading: () => (
    <div className="w-full flex items-center justify-center" style={{ height: "100vh", background: "#0a0a1a" }}>
...
export default function LandingPage() {
  return (
    <main className="bg-[#0a0a1a] text-white overflow-x-hidden">
      ...
      {/* ── Feature Cards ── */}
      <section className="grid grid-cols-1 md:grid-cols-3 gap-5 px-6 pb-24 max-w-5xl mx-auto">
        {[
          { icon: "🛰️", title: "NASA Black Marble",    desc: "Gerçek uydu gece görüntüsünden enerji anomali haritası." },
          { icon: "✦",  title: "Fal AI Görüntü İşleme", desc: "Yüklenen gökyüzü fotoğrafından ışık kirliliği anında siliniyor." },
          { icon: "💡", title: "Akıllı Tasarruf",       desc: "AI önerisiyle yıllık %35 enerji tasarrufu, somut TL karşılığıyla." },
        ].map((c) => (
          <div key={c.title} className="bg-gray-900/60 border border-gray-800 rounded-2xl p-6 hover:border-amber-500/20 transition">
