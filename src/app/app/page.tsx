  ...
  useEffect(() => {
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        ...
        () => {
          setLocationWarning("Doğru konum bilgisi için konum izni gereklidir.");
        }
      );
    }
  }, []);
      ...
      <div className="absolute bottom-0 left-0 right-0 z-10 flex flex-col items-center pb-safe-bottom pb-8 gap-2 pointer-events-none">
        {role === "citizen" ? (
          <button
            ...
            className="pointer-events-auto px-8 py-4 bg-amber-500 hover:bg-amber-400 active:scale-95 text-black font-bold rounded-2xl shadow-xl shadow-amber-500/30 transition text-base flex items-center gap-2"
          >
